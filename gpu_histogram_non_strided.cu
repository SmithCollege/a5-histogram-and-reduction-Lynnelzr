...
__global__ void gpuHistogramNonStrided(int* input, int* histogram, int n, int numBins) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < n) {
        int bin = input[tid] % numBins;
...
int main() {
    ...
    for (int arraySize = 1024; arraySize <= 1048576; arraySize *= 2) {
        ...
        // Launch kernel
        gpuHistogramNonStrided<<<(arraySize + 255) / 256, 256>>>(d_data, d_histogram, arraySize, numBins);
